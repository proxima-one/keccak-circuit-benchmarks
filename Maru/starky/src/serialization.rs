...
pub trait Write {
        ...
        F: RichField + Extendable<D>,
    {
        ...
        if os.permutation_zs.is_some() {
            self.write_field_ext_vec::<F, D>(os.permutation_zs.as_ref().unwrap())?;
        }
        if os.permutation_zs_next.is_some() {
            self.write_field_ext_vec::<F, D>(os.permutation_zs_next.as_ref().unwrap())?;
        }
        ...
        C: GenericConfig<D, F = F>,
    {
        ...
        if proof.permutation_zs_cap.is_some() {
            self.write_merkle_cap(proof.permutation_zs_cap.as_ref().unwrap())?;
        }
