...
use starky::util::{indices_arr, transmute_no_compile_time_size_checks};
...
pub struct KeccakSpongeColumnsView<T: Copy> {
    ...
    pub xored_rate_u32s: [T; KECCAK_RATE_BYTES],

    /// The entire state , encoded as 32-bit chunks, after the current block is xor'd in,
    /// but before the permutation is applied.
    pub xored_state_u32s: [T; KECCAK_WIDTH_U32S],
...
pub const KECCAK_SPONGE_COL_MAP: KeccakSpongeColumnsView<usize> = make_col_map();
