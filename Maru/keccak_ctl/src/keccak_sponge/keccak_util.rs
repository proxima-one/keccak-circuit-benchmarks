...
pub fn keccakf_u32s(state_u32s: &mut [u32; KECCAK_WIDTH_U32S]) {
...
pub fn keccakf_u8s(state_u8s: &mut [u8; KECCAK_WIDTH_BYTES]) {
...
pub fn u8_to_u32_reverse(vecu8: &[u8]) -> Vec<u32> {
    assert!(vecu8.len() >= 4);
    assert_eq!(vecu8.len() % 4, 0);
    let mut vecu32: Vec<u32> = Vec::new();
    for i in (0..vecu8.len()).step_by(4) {
        let a: u32 = ((vecu8[i + 3] as u32) << 24)
            | ((vecu8[i + 2] as u32) << 16)
            | ((vecu8[i + 1] as u32) << 8)
            | (vecu8[i] as u32);
        vecu32.push(a);
